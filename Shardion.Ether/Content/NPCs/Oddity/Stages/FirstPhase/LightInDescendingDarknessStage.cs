...
using Shardion.Ether.Content.Projectiles.Visual;

namespace Shardion.Ether.Content.NPCs.Oddity.Stages.FirstPhase
{
    public class LightInDescendingDarknessStage : OddityStage
    {
        ...
        public override void OnStartAI(Oddity oddity)
        {
            if (!_doneStarting)
            {
                ...
                _ultimateTruth = Projectile.NewProjectileDirect(oddity.NPC.GetSource_FromAI(), _ultimateTruthPosition, Vector2.One, ModContent.GetInstance<UltimateTruth>().Type, 10, 10, oddity.NPC.whoAmI);
            }
            _doneStarting = true;
        }
